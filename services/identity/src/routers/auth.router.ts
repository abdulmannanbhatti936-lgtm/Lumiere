...
import { loginSchema, signupSchema, publishEvent } from '@lumiere/services-shared';
...
export const authRouter = router({
  signup: publicProcedure.input(signupSchema).mutation(async ({ input, ctx }) => {
    ...
    void publishEvent(env.RABBITMQ_URL, 'user.registered', {
      userId: user.id,
      email: user.email,
      name: user.name ?? user.email,
      registeredAt: user.createdAt.toISOString(),
    }).catch((err) => console.error('[identity] failed to publish user.registered:', err));
